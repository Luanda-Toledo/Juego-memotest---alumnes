...
def crear_tarjeta(nombre_imagen: str, identificador: int, nombre_imagen_escondida: str, x: int, y: int) -> dict:
    ...
    img = pygame.image.load(f"recursos/{nombre_imagen}")
    tarjeta["superficie"] = pygame.transform.scale(img, (ANCHO_PANTALLA / CANTIDAD_TARJETAS_H, ALTO_PANTALLA / CANTIDAD_TARJETAS_V))
    img_escondida = pygame.image.load(f"recursos/{nombre_imagen_escondida}")
    tarjeta["superficie_escondida"] = pygame.transform.scale(img_escondida, (ANCHO_PANTALLA / CANTIDAD_TARJETAS_H, ALTO_PANTALLA / CANTIDAD_TARJETAS_V))
    tarjeta["identificador"] = identificador
    tarjeta["visible"] = False
    tarjeta["descubierto"] = False
    tarjeta["rectangulo"] = tarjeta["superficie"].get_rect(topleft = (x, y))
